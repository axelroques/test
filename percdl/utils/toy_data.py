...
class ToyData:
    ...
    def __init__(self, f, N, S, K, L, n) -> None:
        """
    ...
    def generate(self):
        ...
        for s in range(self.S):
            ...
            for k in range(self.K):
                ...
                # Loop over activation values
                activations = self.Z[s, k, :]
                for i, activation in enumerate(activations):
                    if activation:
                        # Create base function shape
                        if k == 0:
                            x = self.first_pattern()
                        if k == 1:
                            x = self.second_pattern()
                        # Transform base shape
                        self.X[s, i:i+self.L] += self.f.transform(x, alphas)
